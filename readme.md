...
This module allows an order to switch between inclusive and exclusive tax modes during
checkout based on a session variable, and provides a custom product type called DynamicTaxModeProduct
that allows setting both normal (inclusive) pricing and business (exclusive) pricing. The pricing shown to the customer
depends on the tax mode currently active.
...
Create a new product with the type `DynamicTaxModeProduct`, or edit an existing product and
...
These work the same as the default `getPricing` and `savePricing` methods for the standard price.
